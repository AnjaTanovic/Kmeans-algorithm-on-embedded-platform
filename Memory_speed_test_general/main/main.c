...
EXT_RAM_BSS_ATTR uint8_t psramBuffer[NUM_OF_BYTES_IN_FILE];							
#endif
...
void testPSRAM() {
    ...
    for (uint32_t i = 0; i < NUM_OF_BYTES_IN_FILE; i++) {
        psramBuffer[i] = i;
    }
    ...
    for (uint32_t size = 0; size < NUM_OF_BYTES_IN_FILE/CHUNK_SIZE; size++) {
        ...
        for (uint16_t i = 0; i < CHUNK_SIZE; i++)
           buffer[i] = psramBuffer[size * CHUNK_SIZE + i];
    ...
    for (uint32_t size = 0; size < NUM_OF_BYTES_IN_FILE/CHUNK_SIZE; size++) {
        ...
        //write chunk size
        for (uint16_t i = 0; i < CHUNK_SIZE; i++)
            psramBuffer[size * CHUNK_SIZE + i] = i;
